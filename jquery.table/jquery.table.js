...
(function($) {
    ...
    $.widget("ui.table", {
        options: {
            ...
            action_pre: '<td><span class="table_action ui-icon ui-widget ui-button $class" action="$action">',
            action_post: '</span></td>',
            default_controls: [{
                    id: 'first',
            ...
            actions: []      // liste des actions ex : [{action:'test',class:'ui-icon-close'}]
        },
        _init: function() {
        },
        /**
        ...
        _create: function() {
            ...
            this.getValues();
            var self = this;
            // tous les controles ont la classe table_controls
            // on leur rajoute l'appel de la methode correspondante
            $("#" + this._getId('navBar')).on('click', ".table_controls",
                    function() {
                        var methode = $(this).attr('id').replace(new RegExp('^' + self.options.name + '_'), '');
                        self[methode]();
                    });
            $("#" + this._getId('table')).on('click', '.table_action', function(event) {
                var action = $(this).attr('action');
                self.call(event.target ,action);
                //$(this._getId('table'))[action]();
            });
        ...
        _createHeader: function() {
            ...
            var nbActions = this.options.actions.length;
            if (nbActions > 0) {
                chaine += '<th colspan="' + nbActions + '">actions</th>';
            }
            $ctrl_id.append(chaine);
            // click sur le titre pour tri
            $ctrl_id.on('click', '.title_div', function() {
                var current_sort = '';
            ...
            $ctrl_id.on('click', '.filter', function() {
        ...
        call: function(element, method) {
            var tr = $(element).closest('tr');
            var data = tr.data('table_data');
            this.options[method](data, tr);
        },
        // on a récupéré les valeurs (donnée en ajax)
        // on met à jour l'écran
        _setValues: function(values) {
            var chaine = '';
            // TODO ajouter les actions
            var chaineAction = "";
            if (this.options.actions.length > 0) {
                for (var k in this.options.actions) {
                    chaineAction += this._template('action_pre', {class: this.options.actions[k].icon, action: this.options.actions[k].name});
                }
            }
            //$('.' + this._getId("values")).remove();
            var $table = $('#' + this._getId('table'));
            $table.find('.table_values').remove();
            for (var i = 0; i < values.length; i++) {
                chaine = "<tr id='lg_"+i+"' class='table_values'>";
                for (var j = 0; j < values[i].length; j++) {
                    chaine += '<td>' + values[i][j] + '</td>';
                }
                chaine += chaineAction += '</tr>';
                $table.append(chaine);
                $('#lg_'+i).data('table_data',values[i]);
            }
            $table.find('.table_values:odd').addClass(this.options.oddclass);
            $table.find('.table_values:even').addClass(this.options.evenclass);
